...
\title{Quality Verification of Digital TV Signals in Production Line Environment}
...
\author{
\IEEEauthorblockN{Paulo Seixas, Pamela Levy, Thales Souza, and Eddie B. de Lima Filho\\}
\IEEEauthorblockA{FPF Tech \\
									Av. Governador Danilo de Matos Areosa, 1170, Dist. Industrial\\
									Manaus-AM, Brazil 69075-351\\
                  Email: \{paulo.seixas,pamela.levy,thales.souza,eddie.filho\}@fpf.br}
}
...
% in the abstract
\begin{abstract}
Production lines normally present a verification step, which is responsible for warning about defective devices, which should not be released for commercial use. That step, in digital television (DTV) production lines, often includes a tuning and decoding check, which basically verifies if DTV devices are capable of presenting audio and video. However, poor signal conditions may cause unnecessary rejections, which happen due to very hostile environments. Because of that, the present paper proposes a verification methodology that reports poor signal conditions during tuning tests, with the goal of reporting that the associated check must be executed again. Tests in a real environment showed that the proposed approach is effective and efficient, while revealed that less than $40\%$ of the obtained rejections were indeed caused by hardware problems.
\end{abstract}
...
When devices in production arrive at the verification workstation, the mentioned tuning test is performed and, at the same time, the monitoring module decides if the signal is in condition to be decoded. Based on the responses from the product line's verification procedure and the monitoring module, the classifier will finally judge if the tuning verification must be repeated or not. Of course, if the signal quality considered unsuitable for a long time, a warn signal is issued, which will advise the operator to check the external test signal. As one may notice, the tuning verification has an integrated signal condition evaluation  (the proposed approach) and, if poor condition is detected, the proposed methodology indicates that another test must be performed. 
...
\bibitem{keith}
K.~Jack, \emph{Video Demystified: A Handbook for the Digital Engineer}, 4th~ed.\hskip 1em plus
  0.5em minus 0.4em\relax Burlington, USA: Newnes, 2005.

\bibitem{reimers}
H.~Reimers, \emph{DVB - The Family of International Standards for Digital Video Broadcasting}.\hskip 1em plus
  0.5em minus 0.4em\relax Heidelberg, Germany: Springer, 2004.

\bibitem{iso13818_1} ISO/IEC 13818-1, \emph{Information technology - Generic coding of moving pictures and associated audio information: systems}. International Organization for Standardization, 2000.

\bibitem{dvb} ETSI EN 300-744, \emph{Digital Video Broadcasting (DVB);
Framing structure, channel coding and modulation for digital terrestrial television}. European Telecommunications Standards Institute, 2009.

\bibitem{isdb} ARIB STD-B31, \emph{Transmission System for Digital Terrestrial Television Broadcasting}. Association of Radio Industries and Business, 2005.

\bibitem{atsc} ATSC A/53, \emph{ATSC Digital Television Standard – Part 2:
RF/Transmission System Characteristics}. Advanced Television Systems Committee, 2011.

\bibitem{fcc} ATSC A/74, \emph{ATSC Recommended Practice: Receiver Performance Guidelines}. Advanced Television Systems Committee, 2010.

\bibitem{sbtvd} ABNT NBR 15604, \emph{Digital terrestrial television - receivers}. Brazilian Association of Technical Standards, 2015.
